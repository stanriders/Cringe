...
namespace Cringe.Bancho.Bancho.RequestPackets
{
    public class MatchReady : RequestPacket
    {
        ...
        public override Task Execute(PlayerSession session, byte[] data)
        {
            ...
            if (slot is null)
            {
                Logger.LogError("{Token} | User tries to set ready status while his MatchSession is null or user hasn't been founded in slots. Match info: {@Session}", session.Token, session.MatchSession);
